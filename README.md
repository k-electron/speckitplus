# SpecKitPlus: SDLC Lifecycle State Tracker

[![Version](https://img.shields.io/badge/version-1.1.1-blue.svg)](CHANGELOG.md)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Dependencies](https://img.shields.io/badge/dependencies-Zero%20(Python%203%20stdlib)-success.svg)](scripts/lifecycle-engine.py)
[![Spec Kit Compatibility](https://img.shields.io/badge/speckit-%3E%3D0.1.0-brightgreen.svg)](extension.yml)
[![CI Quality Gates](https://github.com/k-electron/speckitplus/actions/workflows/ci.yml/badge.svg)](https://github.com/k-electron/speckitplus/actions/workflows/ci.yml)
[![Release Extension](https://github.com/k-electron/speckitplus/actions/workflows/release.yml/badge.svg)](https://github.com/k-electron/speckitplus/actions/workflows/release.yml)

> **A zero-dependency Spec Kit extension that turns every specification, bug, and product assessment into a self-documenting, living state artifact.**

Spec Kit organizes software delivery into markdown specifications (`spec.md`, `plan.md`, `tasks.md`). But knowing where an item stands, how long milestones took, whether a mid-flight agent crashed, or what command to run next often requires manual inspection.

**SpecKitPlus** acts as an automatic, non-intrusive **State Keeper**: it records execution history, passively senses manual edits, detects interruptions, and highlights the next recommended step—persisted directly into your repository filesystem.

---

## Quickstart

### Prerequisites
- [Spec Kit](https://github.com/github/spec-kit) CLI (`specify`) installed and initialized in your repository (`specify init`).
- Python 3.10+ (standard library only; no pip packages or virtual environments needed).

### Installation

Install via the method that matches your setup:

```bash
# Method A: From the Spec Kit community catalog (recommended once published)
specify extension add lifecycle

# Method B: Direct from GitHub release archive
specify extension add lifecycle --from https://github.com/k-electron/speckitplus/releases/download/v1.1.1/lifecycle-1.1.1.zip

# Method C: From a local clone or development branch
specify extension add /path/to/speckitplus --dev
```

### Verify Installation

Confirm that the extension and its commands are registered:

```bash
specify extension list
```

You should see:
```text
✓ SDLC Lifecycle State Tracker (v1.1.1)
   lifecycle
   Living SDLC state artifact tracker and workspace overview
   Commands: 2 | Hooks: 34 | Priority: 10 | Status: Enabled
```

---

## Using SpecKitPlus

**You do not need to change how you work.** SpecKitPlus runs silently in the background via Spec Kit hooks.

### 1. Work as Usual
Run standard Spec Kit commands in your AI agent chat or terminal:

```bash
/speckit-specify "Build modern user notifications"
```

The extension intercepts the command, logs the start timestamp, and generates `specs/002-build-user-notifications/lifecycle.md` upon completion with exact execution metrics.

### 2. Check "What Do I Run Next?"
Any developer or AI coding agent can query the current item health and next recommended action:

```bash
/speckit-lifecycle-status
```

Output:
```text
SDLC Status:   Build Modern User Notifications (002-build-user-notifications)
Track:         Feature
Current Phase: SPECIFIED
Status:        ACTIVE
Next Action:   /speckit-plan (Create architecture and implementation plan)
Progress:      0% (0/0 tasks)
Last Updated:  2026-09-02T10:05:00Z
```

### 3. Progress Through the SDLC
Follow the recommended actions:
1. Run `/speckit-plan` &rarr; `lifecycle.md` updates to `PLANNED`; next action becomes `/speckit-tasks`.
2. Run `/speckit-tasks` &rarr; `lifecycle.md` updates to `TASKED`; next action becomes `/speckit-implement`.
3. Run `/speckit-implement` &rarr; `lifecycle.md` tracks real-time progress:

```text
SDLC Status:   Build Modern User Notifications (002-build-user-notifications)
Current Phase: IMPLEMENTING
Status:        ACTIVE
Next Action:   /speckit-implement (Continue implementation tasks (60% complete))
Progress:      60% (6/10 tasks)
```

As tasks in `tasks.md` are marked `[x]`, progress updates dynamically on every run.

### 4. View the Team Dashboard
Compile or view repository-wide progress across all features, bugs, and idea assessments:

```bash
/speckit-lifecycle-overview
```

This compiles a clean markdown dashboard saved to `.specify/lifecycle-overview.md`:

```markdown
# Repository SDLC Overview

| Track | Active Items | Completed Items |
|---|---|---|
| **Features** | 2 | 5 |
| **Bugs** | 1 | 3 |
| **Assessments** | 0 | 2 |

## Active Work

| Slug | Track | Current Phase | Progress | Next Recommended Action | Last Updated |
|---|---|---|---|---|---|
| 002-build-user-notifications | Feature | IMPLEMENTING | 60% (6/10) | `/speckit-implement` | 2026-09-02 10:45 |
| bug-041-token-expiry | Bug | ASSESSED | 0% (0/0) | `/speckit-bug-fix` | 2026-09-02 09:30 |
```

---

## Core Capabilities

- **Dynamic Title Synchronization**: Automatically extracts and synchronizes canonical feature titles from `spec.md` into `lifecycle.md` and repository dashboards as soon as `/speckit-specify` completes, filters out template placeholders (`[FEATURE NAME]`), seamlessly tracks downstream title edits, and safely bypasses previously converged features during new feature bootstrapping.
- **Crash & Interruption Recovery**: If an AI coding session crashes, terminal times out, or a command aborts mid-flight, running `/speckit-lifecycle-status` flags `Status: INTERRUPTED`, logs elapsed execution time, and provides the exact resumption command without losing completed work.
- **Non-Destructive Soft Drift & Pairwise Sensing**: Directly compares filesystem modification times (`os.path.getmtime`) pairwise between adjacent artifacts (`spec.md` vs `plan.md` and `plan.md` vs `tasks.md`) with a 1.0s clock-skew buffer. Out-of-band edits to upstream specifications increment revision counts and raise an informational `Drift Notice` without deleting downstream work, automatically clear when downstream artifacts catch up, and protect completed milestones (`CONVERGED`, `VERIFIED`) from phase latching.
- **Task Checkbox Synchronization**: Dynamically scans `- [ ]` and `- [x]` items in `tasks.md` on every invocation to keep progress metrics accurate in real time.
- **Zero-Configuration Adoption**: If installed in an existing repository with pre-existing features, running `/speckit-lifecycle-status` or `./scripts/lifecycle-engine.py reconcile` inspects existing files (`spec.md`, `plan.md`, `tasks.md`, `checklists/`) and automatically reconstructs a complete, valid `lifecycle.md`.
- **Zero Third-Party Dependencies**: Pure Python 3 standard library (`sys`, `os`, `json`, `re`, `pathlib`, `datetime`, `argparse`) and POSIX bash. No virtual environments, pip packages, or compilers required.

---

## Track & Command Reference

SpecKitPlus provides native lifecycle taxonomies across three delivery tracks:

### Feature Track (`specs/<slug>/lifecycle.md`)
| Milestone | Trigger Command | Resulting Phase | Next Recommended Action |
|---|---|---|---|
| Initialize | `/speckit-specify` | `SPECIFIED` | `/speckit-plan` |
| Clarify *(optional)* | `/speckit-clarify` | `CLARIFIED` | `/speckit-plan` |
| Checklist *(optional)* | `/speckit-checklist` | `CHECKLISTED` | `/speckit-plan` |
| Plan | `/speckit-plan` | `PLANNED` | `/speckit-tasks` |
| Tasks | `/speckit-tasks` | `TASKED` | `/speckit-implement` |
| Sync Issues *(optional)* | `/speckit-taskstoissues` | `ISSUES_SYNCED` | `/speckit-implement` |
| Analyze *(optional)* | `/speckit-analyze` | `ANALYZED` | `/speckit-implement` |
| Implement | `/speckit-implement` | `IMPLEMENTING` | `/speckit-converge` |
| Converge | `/speckit-converge` | `CONVERGED` | *Complete* |

### Bug Triage Track (`.specify/bugs/<slug>/lifecycle.md`)
| Milestone | Trigger Command | Resulting Phase | Next Recommended Action |
|---|---|---|---|
| Assess | `/speckit-bug-assess` | `ASSESSED` | `/speckit-bug-fix` |
| Fix | `/speckit-bug-fix` | `FIXED` | `/speckit-bug-test` |
| Verify | `/speckit-bug-test` | `VERIFIED` | *Resolved* |
| Escalate | `/speckit-bug-escalate` | `ESCALATED_TO_FEATURE` | `/speckit-specify` (Feature hand-off) |

### Idea Assessment Track (`.specify/assessments/<slug>/lifecycle.md`)
| Milestone | Trigger Command | Resulting Phase | Next Recommended Action |
|---|---|---|---|
| Intake | `/speckit-assess-intake` | `INTAKE` | `/speckit-assess-research` |
| Research | `/speckit-assess-research` | `RESEARCHED` | `/speckit-assess-define` |
| Define | `/speckit-assess-define` | `DEFINED` | `/speckit-assess-shape` |
| Shape | `/speckit-assess-shape` | `SHAPED` | `/speckit-assess-decide` |
| Decision (Go) | `/speckit-assess-decide` | `DECIDED_GO` | `/speckit-specify` (Feature hand-off) |
| Decision (Kill) | `/speckit-assess-decide` | `DECIDED_KILL` | *Archived* |

---

## Anatomy of `lifecycle.md`

Each managed item receives a living `lifecycle.md` combining human-readable GitHub Flavored Markdown with machine-actionable YAML frontmatter:

```markdown
# SDLC Lifecycle: User Authentication
**Track**: Feature | **Current Phase**: `IMPLEMENTING` | **Status**: `ACTIVE`

> [!TIP]
> **Next Recommended Action**: `/speckit-implement`  
> *Continue implementation tasks (60% complete)*

```mermaid
graph LR
    S["1. Specify<br/>✓ Done"] --> P["2. Plan<br/>✓ Done"]
    P --> T["3. Tasks<br/>✓ Done"]
    T --> I["4. Implement<br/>▶ ACTIVE (60%)"]
    I -.-> V["5. Converge<br/>Pending"]
```

## Milestone Timeline

| Phase | Command / Source | Status | Started | Completed | Duration | Notes |
|---|---|---|---|---|---|---|
| **Specify** | `/speckit-specify` | `COMPLETED` | 10:00:00 | 10:02:30 | 2m 30s | Initial specification |
| **Plan** | `/speckit-plan` | `COMPLETED` | 10:15:00 | 10:20:00 | 5m 00s | Architecture approved |
| **Tasks** | `/speckit-tasks` | `COMPLETED` | 10:25:00 | 10:28:00 | 3m 00s | 10 tasks generated |
| **Implement** | `/speckit-implement` | `IN_PROGRESS` | 10:30:00 | — | — | 6/10 tasks completed |
```

The underlying YAML frontmatter stores schema-validated transition records, execution timestamps, duration in seconds, task progress counters, drift advisories, and deviation explanations.

---

## CLI & Script Usage

The engine can also be executed directly in CI pipelines or terminal scripts without Spec Kit slash commands:

```bash
# Query active status (or target a specific directory):
./scripts/lifecycle-engine.py status [--dir specs/001-feature] [--json]

# Compile repository overview (or emit JSON for dashboards):
./scripts/lifecycle-engine.py overview [--all] [--json]

# Reconstruct / reconcile a missing lifecycle.md:
./scripts/lifecycle-engine.py reconcile [specs/001-feature] [--json]
```

---

## Configuration

Optionally configure extension behavior in `.specify/lifecycle.config.yml` (scaffolded from [`config-template.yml`](config-template.yml)):

```yaml
passive_sensing:
  enabled: true
  drift_threshold_seconds: 60  # Grace period to prevent false drift on multi-file saves
overview:
  path: ".specify/lifecycle-overview.md"
  auto_refresh: true
  include_completed: false
interruption_detection:
  enabled: true
format:
  render_mermaid: true
```

---

## How It Works (Under the Hood)

### Philosophy: State Keeper, Never an Enforcer
Traditional workflow tools block developers when steps happen out of order. SpecKitPlus never halts execution or rejects non-standard paths:
- **Descriptive, not prescriptive**: If you skip clarification or jump straight to implementation, it records what actually happened and generates an *Observed Deviation* explanation.
- **Non-destructive layering**: Editing an upstream specification increments revision counts and raises a *Soft Drift Advisory* without wiping out downstream tasks.
- **Filesystem encapsulation**: State lives inside each item directory (`lifecycle.md`), keeping the root clean and the global overview (`.specify/lifecycle-overview.md`) bloat-free.

### Architecture: Dual-Engine Design

```mermaid
graph TD
    subgraph Active["Active Engine (Spec Kit Hooks)"]
        Pre["Pre-Command Hook"] --> Start["Record IN_PROGRESS & Started Timestamp"]
        Post["Post-Command Hook"] --> Complete["Record COMPLETED, Duration & Exit Code"]
    end

    subgraph Passive["Passive Engine (Artifact Sensing)"]
        FileMod["Manual / Conversational Edits"] --> Sense["Timestamp & Hash Scanner"]
        Tasks["tasks.md Checkboxes"] --> Prog["Real-Time Completion %"]
    end

    Core["Lifecycle Engine (Python 3 stdlib)"]

    subgraph Storage["Persisted State Artifacts"]
        LivingDoc["Living Artifact (specs/[slug]/lifecycle.md)"]
        Overview["Repository Dashboard (.specify/lifecycle-overview.md)"]
    end

    Start --> Core
    Complete --> Core
    Sense --> Core
    Prog --> Core
    Core --> LivingDoc
    Core --> Overview
```

1. **Active Engine**: Intercepts Spec Kit commands across all milestones (core, optional refinements, bug triage, and idea assessments) to log start/end timestamps, elapsed durations, exit codes, and phase transitions.
2. **Passive Engine**: Inspects filesystem mtimes to detect conversational edits in chat or IDEs without slash commands, and calculates live `- [x]` completion ratios from `tasks.md`.
3. **State Reconciliation**: If a `lifecycle.md` is deleted or pre-dates extension installation, scanning existing artifacts (`spec.md`, `plan.md`, `tasks.md`, `checklists/`) automatically reconstructs the full milestone timeline.

---

## Contributing

We welcome contributions! Please see [CONTRIBUTING.md](CONTRIBUTING.md) for development practices, test suite execution, and release workflows. AI coding agents should refer to [AGENTS.md](AGENTS.md) for operational constraints.

---

## License

This project is licensed under the [MIT License](LICENSE).
